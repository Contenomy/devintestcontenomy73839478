...
const PaymentSummaryPage: React.FC = () => {
  ...
  const processPaymentWallet = async (cardId:string) => {
    ...
    try {
      ...
      if (paymentResponse.ok) {
        const returnUrl = `${window.location.origin}/payment-order/TransferConfirmation?status=${paymentResult.status}`;
        window.location.href = returnUrl;  

        
      } else {
        const returnUrl = `${window.location.origin}/payment-order/TransferConfirmation?status=ERROR`;
        window.location.href = returnUrl;
      }
    } catch (error) {
      const returnUrl = `${window.location.origin}/payment-order/TransferConfirmation?status=ERROR`;
  ...
  const processPayment = async (cardId:string) => {
    ...
    try {
      const paymentResponse = await fetch('https://localhost:7126/api/Account/CreatePaymentPreauth', {
        credentials: 'include',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
      ...
      if (paymentResponse.ok) {
        if (paymentResult.status === "FAILED") {
          ...
          sessionStorage.setItem('orderData_' + paymentResult.id, JSON.stringify(items[0].order));
        ...
        else if (paymentResult.status === "SUCCEEDED") 
        {
          ...
          const secureRedirectUrl = `${window.location.origin}/payment-order/PaymentConfirmation?preAuthorizationId=${paymentResult.id}&status=${paymentResult.status}`;
          window.location.href = secureRedirectUrl;
      ...
      if (registrationResult.resultMessage == "Success") {
        ...
        if (paymentResponse.ok) {
          if (paymentResult.status === "FAILED") {
            ...
            const returnUrl = `${window.location.origin}/PaymentConfirmation?payInId=${paymentResult.id}&Order=${items.order}`;
            const secureRedirectUrl = `${paymentResult.secureModeRedirectURL}&returnUrl=${encodeURIComponent(returnUrl)}`;
            window.location.href = secureRedirectUrl;
          }
          else if (paymentResult.status === "SUCCEEDED") 
          {
            const secureRedirectUrl = `${window.location.origin}/payment-order/PaymentConfirmation?status=${paymentResult.status}&Order=${items.order}`;
            window.location.href = secureRedirectUrl;
          }
        } else {
